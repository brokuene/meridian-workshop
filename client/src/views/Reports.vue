  ...
  setup() {
    ...
    const barHeights = computed(() =>
      monthlyData.value.map(m =>
        maxRevenue.value ? (m.revenue / maxRevenue.value) * 200 : 0
      )
    )
    ...
    return {
      ...
      totalRevenue, avgMonthlyRevenue, totalOrders, bestQuarter, barHeights,
      loadData, formatCurrency, formatMonth,
...
.reports-table td { padding: 0.75rem; border-bottom: 1px solid #e2e8f0; }
.reports-table tr:hover { background: #f8fafc; }

.chart-container { padding: 2rem 1rem; min-height: 300px; }
.bar-chart {
  display: flex; align-items: flex-end; justify-content: space-around;
  height: 250px; gap: 0.5rem;
}
.bar-wrapper { display: flex; flex-direction: column; align-items: center; flex: 1; max-width: 80px; }
.bar-container { height: 200px; display: flex; align-items: flex-end; width: 100%; }
.bar {
  width: 100%; background: linear-gradient(to top, #3b82f6, #60a5fa);
...
.bar-label {
  ...
  transform: rotate(-45deg); white-space: nowrap; margin-top: 1.5rem;
}
...
.error {
  ...
  border-radius: 8px; margin: 1rem 0; display: flex;
  align-items: center; justify-content: space-between;
}
.retry-btn {
  background: #991b1b; color: white; border: none; padding: 0.375rem 0.75rem;
